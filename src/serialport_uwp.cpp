...
void EIO_List(uv_work_t* req) {
	...
	for (size_t i = 0; i < deviceInfoCollection->Size; i++) {
		...
		std::wstring comName(deviceInfoCollection->GetAt(i)->Id->Data()); // Use Id instead of Name since Id is used in FromIdAsync
		auto wideData = comName.c_str();
