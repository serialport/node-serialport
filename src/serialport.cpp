...
void Open(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  ...
  baton->dataCallback = new Nan::Callback(options->Get(Nan::New("datacallback").ToLocalChecked()).As<v8::Function>());
  baton->disconnectedCallback = new Nan::Callback(options->Get(Nan::New("disconnectedcallback").ToLocalChecked()).As<v8::Function>());
  baton->errorCallback = new Nan::Callback(options->Get(Nan::New("errorcallback").ToLocalChecked()).As<v8::Function>());
...
void EIO_AfterList(uv_work_t* req) {
  ...
  if(data->errorString[0]) {
    ...
    for(std::list<ListResultItem*>::iterator it = data->results.begin(); it != data->results.end(); ++it, i++) {
      ...
      item->Set(Nan::New("comName").ToLocalChecked(), Nan::New((*it)->comName.c_str()).ToLocalChecked());
      item->Set(Nan::New("manufacturer").ToLocalChecked(), Nan::New((*it)->manufacturer.c_str()).ToLocalChecked());
      item->Set(Nan::New("serialNumber").ToLocalChecked(), Nan::New((*it)->serialNumber.c_str()).ToLocalChecked());
      item->Set(Nan::New("pnpId").ToLocalChecked(), Nan::New((*it)->pnpId.c_str()).ToLocalChecked());
      item->Set(Nan::New("locationId").ToLocalChecked(), Nan::New((*it)->locationId.c_str()).ToLocalChecked());
      item->Set(Nan::New("vendorId").ToLocalChecked(), Nan::New((*it)->vendorId.c_str()).ToLocalChecked());
      item->Set(Nan::New("productId").ToLocalChecked(), Nan::New((*it)->productId.c_str()).ToLocalChecked());
      results->Set(i, item);
    }
    argv[0] = Nan::Undefined();
    argv[1] = results;
...
void Set(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  ...
  baton->fd = fd;
  baton->callback = new Nan::Callback(callback);
  baton->rts = options->Get(Nan::New("rts").ToLocalChecked())->ToBoolean()->BooleanValue();
  baton->cts = options->Get(Nan::New("cts").ToLocalChecked())->ToBoolean()->BooleanValue();
  baton->dtr = options->Get(Nan::New("dtr").ToLocalChecked())->ToBoolean()->BooleanValue();
  baton->dsr = options->Get(Nan::New("dsr").ToLocalChecked())->ToBoolean()->BooleanValue();
