...
const loggers = {};
const bindingLogger = (name, message) => {
  const log = loggers[name] = loggers[name] || require('debug')(`serialport:binding:function:${name}`);
  log(message);
};
...
SerialPort.prototype.update = function(options, callback) {
  ...
  bindingLogger('update', 'updating');
  this.binding.update(this.settings).then(() => {
    bindingLogger('update', 'finished');
    if (callback) { callback.call(this, null) }
  }, (err) => {
    bindingLogger('update', 'errored');
...
SerialPort.prototype._write = function(data, encoding, callback) {
  ...
  bindingLogger('write', `${data.length} bytes of data`);
  this.binding.write(data).then(
    () => {
      bindingLogger('write', 'write finished');
      callback(null);
    },
    (err) => {
      bindingLogger('write', 'write error');
      callback(err);
    });
};
...
SerialPort.prototype._read = function(bytesToRead) {
  if (!this.isOpen) {
    debug('_read', 'qeueuing _read for after open');
    this.once('open', () => {
  ...
  // the actual read.
  bindingLogger('read', `reading`);
  this.binding.read(pool, start, toRead).then((bytesRead) => {
    bindingLogger('read', `finished`);
    // zero bytes means read means we've hit EOF?
    if (bytesRead === 0) {
      debug('Zero bytes read closing readable stream');
      this.push(null);
      return;
    }
    pool.used += bytesRead;
...
SerialPort.list = function(cb) {
  ...
  let promise = SerialPort.Binding.list();
  if (cb) {
    promise = promise.then(ports => cb(null, ports), err => cb(err));
  }
  return promise;
};
