...
/**
 * Gets flags on an open port. [`GetCommModemStatus`](https://msdn.microsoft.com/en-us/library/windows/desktop/aa363258(v=vs.85).aspx) for windows and [`ioctl`](http://linux.die.net/man/4/tty_ioctl) for mac and linux.
 * @param {module:serialport~getCallback=} callback Called once the port's flags have been retrieved.
 */
SerialPort.prototype.get = function(callback) {
  ...
  SerialPortBinding.get(this.fd, function(err, status) {
    if (err) {
      debug('SerialPortBinding.get had an error', err);
      return this._error(err, callback);
    }
