...
UwpBinding.prototype.read = function(buffer, offset, length, cb) {
...
UwpBinding.prototype.open = function(path, options, cb) {
  if (!path) {
...
UwpBinding.prototype.close = function(cb) {
  if (typeof cb !== 'function') {
    throw new TypeError('"cb" is not a function');
  }
...
UwpBinding.prototype.set = function(options, cb) {
  if (typeof options !== 'object') {
    throw new TypeError('"options" is not an object');
  }
...
UwpBinding.prototype.write = function(buffer, cb) {
  if (!Buffer.isBuffer(buffer)) {
    throw new TypeError('"buffer" is not a Buffer');
  }
...
commonMethods.map(function(methodName) {
  UwpBinding.prototype[methodName] = function() {
...
Object.defineProperty(UwpBinding.prototype, 'isOpen', {
  enumerable: true,
  get() {
    return this.fd !== null;
  }
...
UwpBinding.list = binding.list;
module.exports = UwpBinding;
