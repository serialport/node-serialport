...
module.exports = class CCTalkParser extends Transform {
  ...
  _transform(buffer, _, cb) {
    ...
    while (this.array.length > 1 && this.array.length >= FullMsgLength) {
      ...
      if (this.array.length > FullMsgLength) { // Preserve Additional data from next Command
        const AdditionalData = this.array.slice(frame.length, this.array.length);
        this.array.splice(0, FullMsgLength, AdditionalData);
        this.array.length = AdditionalData.length;
