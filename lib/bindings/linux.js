...
LinuxBinding.list = require('./linux-list');

LinuxBinding.prototype.read = require('./unix-read');
...
LinuxBinding.prototype.open = function(path, options, cb) {
  ...
  options = Object.assign({}, options, defaultBindingOptions, this.bindingOptions);

  binding.open(path, options, (err, fd) => {
    if (err) {
      return cb(err);
    }
    this.fd = fd;
