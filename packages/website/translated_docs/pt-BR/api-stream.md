...
Arguments: - `size?: number` Specify how many bytes of data to return, if available
...
Arguments: - `callback?: (error => {}: void) Called once a connection is closed.
...
Arguments: - `options: setOptions` All options are operating system default when the port is opened. Every flag is set on each call to the provided or default values. If options isn't provided default options are used. - `callback: error => {}` Called once the port's flags have been set.
