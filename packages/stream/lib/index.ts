...
import { PortInfo, AbstractBinding } from '@serialport/binding-abstract'
const logger = debug('serialport/stream')
...
function allocNewReadPool(poolSize: number) {
...
// export class SerialPort extends stream.Duplex {
//   get isOpen() {
...
//   static Binding: typeof AbstractBinding
...
//   closing: any
//   opening: boolean
//   _pool: { used: number; buffer: Buffer }
...
//    */
//   constructor(path: string, options: any, openCallback?: any) {
...
//   _error(error: Error, callback?: ErrorCallback) {
//     if (callback) {
...
//   _asyncError(error: Error, callback?: ErrorCallback) {
...
//     this.opening = true
//     logger('opening', `path: ${this.path}`)
//     this.binding.open(this.path, this.settings).then(
//       () => {
//         logger('opened', `path: ${this.path}`)
//         this.opening = false
//         this.emit('open')
//         if (openCallback) {
...
//       (err: Error) => {
//         this.opening = false
//         logger('Binding #open had an error', err)
...
//     if (!this.isOpen) {
//       logger('update attempted, but port is not open')
...
//     logger('update', `baudRate: ${settings.baudRate}`)
//     this.binding.update(this.settings).then(
//       () => {
//         logger('binding.update', 'finished')
//         if (callback) {
//           callback.call(this, null)
//         }
//       },
//       err => {
//         logger('binding.update', 'error', err)
...
//       })
//     }
//     logger('_write', `${data.length} bytes of data`)
//     this.binding.write(data).then(
//       () => {
//         logger('binding.write', 'write finished')
//         callback(null)
//       },
//       err => {
//         logger('binding.write', 'error', err)
//         if (!err.canceled) {
...
//     logger('_writev', `${data.length} chunks of data`)
...
//     if (!this.isOpen) {
//       logger('_read', 'queueing _read for after open')
//       this.once('open', () => {
...
//     if (this._pool.buffer.length - this._pool.used < this._kMinPoolSpace) {
//       logger('_read', 'discarding the read buffer pool')
//       this._pool = allocNewReadPool(this.settings.highWaterMark)
//     }
...
//     const toRead = Math.min(pool.buffer.length - pool.used, bytesToRead)
//     const start = pool.used

//     // the actual read.
//     logger('_read', `reading`)
//     this.binding.read(pool.buffer, start, toRead).then(
//       bytesRead => {
//         logger('binding.read', `finished`)
//         // zero bytes means read means we've hit EOF? Maybe this should be an error
//         if (bytesRead === 0) {
//           logger('binding.read', 'Zero bytes read closing readable stream')
//           this.push(null)
//           return
//         }
//         pool.used += bytesRead
//         this.push(pool.buffer.slice(start, start + bytesRead))
//       },
//       err => {
//         logger('binding.read', `error`, err)
//         if (!err.canceled) {
//           this._disconnected(err)
//         }
...
//     if (!this.isOpen) {
//       logger('disconnected aborted because already closed', err)
//       return
//     }
//     logger('disconnected', err)
//     ;(err as any).disconnected = true
//     this.close(null, err)
//   }
...
//    * @returns {undefined}
//    */
//   close(callback: ErrorCallback, error?: Error) {
//     const disconnectError = error || null

//     if (!this.isOpen) {
//       logger('close attempted, but port is not open')
//       return this._asyncError(new Error('Port is not open'), callback)
//     }
...
//       err => {
//         this.closing = false
//         logger('binding.close', 'had an error', err)
//         return this._error(err, callback)
//       }
//     )
//   }
...
//     if (!this.isOpen) {
//       logger('set attempted, but port is not open')
//       return this._asyncError(new Error('Port is not open'), callback)
//     }
...
//       () => {
//         logger('binding.set', 'finished')
//         if (callback) {
//           callback.call(this, null)
//         }
//       },
//       err => {
//         logger('binding.set', 'had an error', err)
//         return this._error(err, callback)
//       }
//     )
//   }
...
//   get(callback) {
//     if (!this.isOpen) {
//       logger('get attempted, but port is not open')
//       return this._asyncError(new Error('Port is not open'), callback)
//     }
...
//       status => {
//         logger('binding.get', 'finished')
//         if (callback) {
//           callback.call(this, null, status)
//         }
//       },
//       err => {
//         logger('binding.get', 'had an error', err)
//         return this._error(err, callback)
//       }
//     )
//   }
...
//   flush(callback) {
//     if (!this.isOpen) {
//       logger('flush attempted, but port is not open')
//       return this._asyncError(new Error('Port is not open'), callback)
//     }
...
//       () => {
//         logger('binding.flush', 'finished')
//         if (callback) {
//           callback.call(this, null)
//         }
//       },
//       err => {
//         logger('binding.flush', 'had an error', err)
//         return this._error(err, callback)
//       }
//     )
//   }
...
//     logger('drain')
//     if (!this.isOpen) {
//       logger('drain queuing on port open')
...
//       () => {
//         logger('binding.drain', 'finished')
//         if (callback) {
//           callback.call(this, null)
//         }
//       },
//       err => {
//         logger('binding.drain', 'had an error', err)
//         return this._error(err, callback)
//       }
//     )
//   }
...
// /**
//  * This callback type is called `listCallback`.
